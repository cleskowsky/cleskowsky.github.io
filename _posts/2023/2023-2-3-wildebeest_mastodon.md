---
layout: post
title: "WildeBeest Mastodon Compatible Personal Fediverse Server"
---

* Somebody @ Cloudflare is writing a fediverse server as a proof of concept for their serverless platform.
* I think the db is backed by sqlite3 which is amazing.
* [Github](https://github.com/cloudflare/wildebeest)

* Nice example of relational schema for activitypub data model. Straight forward to follow. You can learn a lot about an application by looking at its database.
* UUIDs are used exclusively for primary keys. UUIDv4s are not guaranteed to be unique in theory but the chances of a collision are incredibly small that in practice most datasets will be just fine using them vs auto-incrementing counters.
    * UUIDs are nice when you need unique ids generated across a distributed db with multiple shards.
    * Auto-incrementing numbers can be revealing in terms of how many rows are in a table and they certainly wouldn't be unique across a sharded db.
